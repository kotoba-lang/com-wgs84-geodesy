(ns wgs84_geodesy.main
  "Kotodama WASM entrypoint for the wgs84_geodesy clean-room actor (L4) — Clojure port.
  ...
  facts (`wgs84_geodesy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "wgs84_geodesy")
(def tier "L4")
...
(def entity-specs
  [{:entity "Code"      :plural "codes"      :id-prefix "wgs84geo_cod"
    :fields [:code :description :scheme :parentCode] :required [:code :description]
    :coerce {}                       :refs {}}
   {:entity "Unit"      :plural "units"      :id-prefix "wgs84geo_uni"
    :fields [:symbol :name :quantity]       :required [:symbol :name]
    :coerce {}                       :refs {}}
   {:entity "Reference" :plural "references" :id-prefix "wgs84geo_ref"
    :fields [:name :value :uncertainty :unit] :required [:name :value]
    :coerce {:value :float :uncertainty :float} :refs {}}
   {:entity "Mapping"   :plural "mappings"   :id-prefix "wgs84geo_map"
    :fields [:fromCode :toScheme :toCode]   :required [:fromCode :toScheme]
    :coerce {}                       :refs {}}
   {:entity "Location"  :plural "locations"  :id-prefix "wgs84geo_loc"
    :fields [:unLocode :name :country]      :required [:unLocode :name]
    :coerce {}                       :refs {}}
   {:entity "Epoch"     :plural "epoches"    :id-prefix "wgs84geo_epo"
    :fields [:name :utcOffset :asOf]        :required [:name :utcOffset]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"wgs84_geodesy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "wgs84_geodesy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :wgs84_geodesy-compat/ready)
