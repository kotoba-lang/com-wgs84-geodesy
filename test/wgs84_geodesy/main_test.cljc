(ns wgs84_geodesy.main-test
  "Contract + behavioral test for the wgs84_geodesy-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "wgs84_geodesy-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'wgs84_geodesy.main-test)]
